...
2. ✅ **DONE — evidence replay/aggregation tool (PR #130, `a64b903`).** `harness/risk_evidence.py`: scans all `sessions/*.jsonl`, aggregates `risk_classified` by `kind`/current-risk/mission. **Re-classifies via `classify_action`** (current taxonomy) rather than trusting the logged risk, and reports logged-vs-current **drift**. CLI: `python risk_evidence.py [--sessions-dir DIR] [--json]`. (Closed the gap a fresh Explore verification found: events persist durably to `<repo>/sessions/<mission_id>.jsonl` — survives across runs, outside the per-run `output_dir` wipe — but nothing read them back.)
3. ⏭️ **Gather evidence (operator-gated, NEXT bottleneck)** — run real builds so the now-complete instrumentation (#128) produces `risk_classified` events, then `python harness/risk_evidence.py` aggregates them. Thresholds must come from this data, not guesses.
4. ⏭️ **Safety vertical slice: #6 enforcement + #1 modes** — evidence-gated + highest-risk (a bad threshold can brick unattended runs) → last. **✅ DESIGN drafted 2026-06-18 (design-apart, no code): `C:\Users\Tyler\.claude\plans\enforcement-gateway-design.md`** — `observe()`→`gate()` decision return (block = graceful skip, never raises); runtime policy from `config.py` mode + per-kind overrides (NOT the evidence file, which is the offline tuning input); attended/unattended `RunContext` with Telegram-approval-or-fail-closed for `ask`; mode×risk matrix; proposed unattended default `auto_safe`; shadow-enforce→critical-first rollout. **Central tension the impl must respect:** `shell`=high but fires every film render → needs per-kind allowlist/override from evidence, not risk-level-only blocking. Implementation still BLOCKED on step-3 evidence.
...
## ✅ DONE 2026-06-18 (eleventh session) — risk-evidence aggregator (PR #130, MERGED)

Roadmap #6 step 2. Merged `a64b903`. New read-only `harness/risk_evidence.py` closes the read-back gap: `observe()` logged durably but nothing aggregated it.
- `iter_risk_events()` — tolerant reader (skips unreadable files, non-JSON lines, truncated final line; session_log appends without fsync).
- `aggregate()` → `{total, missions, by_kind, by_risk, by_kind_risk, by_mission, drift}`. **Risk re-derived from `classify_action(kind, detail)`** (current taxonomy), NOT the logged value — `drift` counts records whose logged risk no longer matches current (e.g. `shell` events logged before the low→high change).
- `format_report()` / `--json` CLI.
- `tests/test_risk_evidence.py`: 3 tests (filtering, malformed-line tolerance, current-taxonomy re-classification + drift, severity ordering). Suite green; CLI on the real (empty) `sessions/` honestly reports "no evidence yet."
