...
**2026-06-15 (seventh session) — PRs #55–#61 merged; Ollama-first protection live; factory rehearsal builds running.** PR #55: context-aware dashboard control buttons. PR #57: worker escalation learning loop (`log_escalation()` + `get_worker_hints()`). PR #58: roadmap follow-up items. PR #59: docs sync. PR #60: Ollama token tracking in the cost panel — `prompt_eval_count`/`eval_count` from Ollama responses now recorded and shown as a "local (ollama)" row alongside cloud spend. PR #61 (critical): `_is_ollama_unavailable()` guard in `worker.py` — an unreachable Ollama server now fails the task immediately with a clear error; previously `ConnectionError` was caught by the generic escalation handler and silently walked the ladder to Sonnet, billing Anthropic for every task (discovered live: $0.50 burned in one run when Ollama was down). 32/32 tests green. Two factory rehearsal builds ran with the full local ladder (qwen3:8b + deepseek-coder-v2:16b): NES portfolio deployed ✅; Tony Montana v8 deployed but healer couldn't auto-fix the `#hero`/`.hero` class mismatch across cycles (NEEDS_FOLLOWUP). Remaining: healer gap fix + factory rehearsal items #3–7.
